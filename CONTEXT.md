...
**Spec harvester**:
The component that scans source and derives Form specs into a registry from several heuristic signals. The portable, highest-yield signal — a def-macro's own arglist parameter names (`name`/`arglist`/`docstring`/`body`) — works across every macro-defining Lisp (Emacs Lisp, Common Lisp, Clojure, Fennel, Janet, Hy, LFE, ISLisp), driven by a per-dialect **harvest profile** (macro-defining heads, docstring policy) via `harvest_source_for` (ADR-0032). Emacs Lisp additionally supplies `declare` metadata (`debug (&define …)`, `doc-string`, `indent`), and Emacs's own definition macros are harvested and bundled as builtins. The Scheme family is out of this harvester's reach — its macros are `syntax-rules` *patterns*, not an arglist, needing a separate pattern harvester (ADR-0031). (Tentatively called "macro-collector".)
